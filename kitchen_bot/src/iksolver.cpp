...
{   
	...
	std::string urdf_path = ros::package::getPath("kitchen_bot");
	if(urdf_path.empty()) {
            
		ROS_ERROR("kitchen_bot package path was not found");
	}
	urdf_path += "/urdf/kitchen_ik.urdf";
    ...
    ROS_INFO("Creating subscribers for all joints");
    ...
    ROS_INFO("Creating publishers to send position commands to all joints");
    ...
    ROS_INFO("Generating kinematic chain from the robot base to its tcp");
	//Create solvers
	...
	for(int i=0; i< 2; i++) {
		ros::spinOnce();
	 	loop_rate.sleep();
        ROS_INFO("Making sure we have received proper joint angles already");
	}
